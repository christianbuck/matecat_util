...
class SymalWrapper(object):
    ...
    def symmetrize(self, source_txt, target_txt, s2t_align, t2s_align):
        print "called symmetrize with:\t%s\n\t%s\n\t%s\n\t%s" % (source_txt.encode("utf-8"), target_txt.encode("utf-8"), s2t_align, t2s_align)
        ...
        s = u"1\n%d %s  # %s\n%d %s  # %s\n" %(t_len, target_txt, s2t_align,
                                              s_len, source_txt, t2s_align)
        print "Input to symal: %s" % s.encode("utf-8")
