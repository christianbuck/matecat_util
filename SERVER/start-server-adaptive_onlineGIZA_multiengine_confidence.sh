...
server=$softwaredir/SERVER/server_adaptive_onlineGIZA_multiengine_confidence.py
...
ip=hlt-services2.fbk.eu
serverThreads=2
serververbose=1
src=en
trg=it
MODELS=$matecatdir/data/UCLegal_system/MODELS-Matecat.Legal.baseline-en-it_20131008
UPDATER=$matecatdir/data/UCLegal_system/UPDATER-Matecat.Legal.baseline-en-it_20131008
QE_MODELS=$matecatdir/QE_data/MateCatData/lang_resources/En-It-Legal-D0

#parameters for Moses
mosescmd=$mosesdir/bin/moses
moses_config=${MODELS}/moses-static.ini
moses1_config=${MODELS}/moses-adaptive.ini

#parameters for Updater
updater_config="_NOUPDATE_"
updater1_config="_NOUPDATE_"

#Systems names
system_name="SYSTEM_baseline"
system1_name="SYSTEM_contrastive1"
...
#parameters to modify size of cache-based-models
fixedmosesoptions="$fixedmosesoptions -cbtm-max-age 10000 -cblm-max-age 10000"
...
exe="$exe -moses $mosescmd -options \"${fixedmosesoptions} -f $moses_config \" -system-name $system_name"
...
#added parameters for (up to four different) Updaters
exe="$exe -updater ${updater_config}"
if [ ${updater1_config} != "" ] ; then exe="$exe  -updater1 ${updater1_config}" ; fi
if [ ${updater2_config} != "" ] ; then exe="$exe  -updater2 ${updater2_config}" ; fi
if [ ${updater3_config} != "" ] ; then exe="$exe  -updater3 ${updater3_config}" ; fi
...
sentenceconfidence_configdir="${QE_MODELS}/resources/TER"
sentenceconfidence_jarfile="$softwaredir/QualityEstimation/QEModule.jar"
sentenceconfidence_tmpdir="$softwaredir/QualityEstimation/TestFolder"
sentenceconfidence_options=""

# QE module requires an pre-existing directory for storing temporary files, 
# which are totally removed after the computation
if [ ! -d $sentenceconfidence_tmpdir ] ; then mkdir -p $sentenceconfidence_tmpdir ; fi

sentenceconfidence_options="$sentenceconfidence_options $sentenceconfidence_configdir/config_SVM.txt"
sentenceconfidence_options="$sentenceconfidence_options $sentenceconfidence_configdir/config.properties"
sentenceconfidence_options="$sentenceconfidence_options $sentenceconfidence_tmpdir"
sentenceconfidence_options="$sentenceconfidence_options false true"
sentenceconfidencecmd="java -Xmx1500m -XX:+UseConcMarkSweepGC  -jar $sentenceconfidence_jarfile $sentenceconfidence_options"

exe="$exe -sentence_confidence "
exe="$exe \"$sentenceconfidencecmd\""
...
preprocesscmd="$softwaredir/code/tokenizer/deescape-special-chars.perl"
exe="$exe \"$preprocesscmd\""

preprocesscmd="$softwaredir/code/tokenizer/tokenizer.perl -b -l $src"
exe="$exe \"$preprocesscmd\""
#END PATCH
...
#parameters for Preprocessing source on the update request
exe="$exe -updater_source_prepro "
...
#parameters for Preprocessing target on the update request
exe="$exe -updater_target_prepro "

#START PATCH
#preprocesscmd="$softwaredir/code/tokenizer/tokenizer.perl -X -b -l $trg"
#exe="$exe \"$preprocesscmd\""

preprocesscmd="$softwaredir/code/tokenizer/deescape-special-chars.perl"
exe="$exe \"$preprocesscmd\""

preprocesscmd="$softwaredir/code/tokenizer/tokenizer.perl -b -l $trg"
exe="$exe \"$preprocesscmd\""
#END PATCH

preprocesscmd="$softwaredir/code/monolingual/accents.pl -b -l $trg"
exe="$exe \"$preprocesscmd\""
